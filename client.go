...
import (
	...
	"strconv"
	"strings"
	ldap "github.com/mybee/goldap/message"
)
...
func (c *client) serve() {
	...
	for {
		...
		if err != nil {
			if strings.Contains(err.Error(), "userAccountControl err") {
				c.writeUserControlMessage()
				return
			}
			//return
...
func (c *client) writeUserControlMessage() {
	//data, _ := m.Write()
	Logger.Printf(">>> %d - %s - hex=%x", "userController", "result", HexToByte("303102010465070a010004000400a02330210416312e322e3834302e3131333535362e312e342e333139040730050201000400"))
	c.bw.Write(HexToByte("303102010465070a010004000400a02330210416312e322e3834302e3131333535362e312e342e333139040730050201000400"))
	c.bw.Flush()
}
